...
- When you are first transferring your HTML to Moodle, the image will naturally not be found.
  ...
  <img class="img-fluid"
    ...
    alt="Example" width="4884" height="1422">
  ```

- Moodle can now (after uploading the image; saving is not necessary) identify the image using the `example.png` filename.
  In fact, when saving the resource, the rest of the URL is removed by Moodle and replaced by `@@PLUGINFILE@@` because Moodle only needs this part for editing.

- When you replace that image by what Moodular created, the preview will no longer display the image (since the `src` changed from a working draft URL on your Moodle site to a placeholder containing `@@PLUGINFILE@@`), but this does not impact how the image is saved when saving the whole resource.
